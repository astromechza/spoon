...
func (a *netAgent) Tick(sink sink.Sink) error {
    ...
    for _, nicio := range iocounters {
        ...
        err = sink.Put(fmt.Sprintf("%s.bytes_recv", prefixPath), float64(nicio.BytesRecv))
        if err != nil { return err }

        err = sink.Put(fmt.Sprintf("%s.packets_sent", prefixPath), float64(nicio.PacketsSent))
        if err != nil { return err }

        err = sink.Put(fmt.Sprintf("%s.packets_recv", prefixPath), float64(nicio.PacketsRecv))
        if err != nil { return err }
